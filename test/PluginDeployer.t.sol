...
contract PluginDeployerTest is Test {
    MockREVDeployer internal mock;
    PluginDeployer internal deployer;
    ...
    function setUp() public {
        ...
        deployer = new PluginDeployer(mock, contexts);
    }

    function _machine(PluginKeep keep, PluginDoubling doubling) internal view returns (PluginMachine memory) {
    ...
    function test_houseConfigAppliesHouseRules() public view {
        REVConfig memory config = deployer.houseConfig(_machine(PluginKeep.A_BIT, PluginDoubling.WEEKLY));
    ...
    function test_startEngineForwardsToREVDeployer() public {
        uint256 revnetId = deployer.startEngine{value: 0.01 ether}(_machine(PluginKeep.HALF, PluginDoubling.DAILY), _emptySuckers());
    ...
    function test_enumMappings() public view {
        assertEq(deployer.keepPercent(PluginKeep.NONE), 0);
        assertEq(deployer.keepPercent(PluginKeep.A_BIT), 1000);
        assertEq(deployer.keepPercent(PluginKeep.A_GOOD_BIT), 3200);
        assertEq(deployer.keepPercent(PluginKeep.HALF), 5000);
        assertEq(deployer.keepPercent(PluginKeep.THE_BULK), 6800);
        assertEq(deployer.keepPercent(PluginKeep.ALL_BUT_A_BIT), 9000);
        assertEq(deployer.doublingSeconds(PluginDoubling.DAILY), 1 days);
        assertEq(deployer.doublingSeconds(PluginDoubling.WEEKLY), 7 days);
        assertEq(deployer.doublingSeconds(PluginDoubling.MONTHLY), 30 days);
        assertEq(deployer.doublingSeconds(PluginDoubling.QUARTERLY), 90 days);
    }

    function test_routesSplitTheKeepWithMachineAsBeneficiary() public view {
        PluginMachine memory m = _machine(PluginKeep.A_BIT, PluginDoubling.WEEKLY);
        m.routes = new PluginRoute[](2);
        m.routes[0] = PluginRoute({projectId: 3, percentOfKeep: 10, locked: true});
        m.routes[1] = PluginRoute({projectId: 4, percentOfKeep: 25, locked: false});
    ...
    function test_revertsWhenRoutesExceedKeep() public {
        PluginMachine memory m = _machine(PluginKeep.A_BIT, PluginDoubling.WEEKLY);
        m.routes = new PluginRoute[](2);
        m.routes[0] = PluginRoute({projectId: 3, percentOfKeep: 60, locked: false});
        m.routes[1] = PluginRoute({projectId: 4, percentOfKeep: 41, locked: false});
        vm.expectRevert(abi.encodeWithSelector(PluginDeployer.PluginDeployer_RoutesExceedKeep.selector, 101));
        deployer.houseConfig(m);
    }

    function test_revertsOnBadInputs() public {
        PluginMachine memory noMachine = _machine(PluginKeep.NONE, PluginDoubling.DAILY);
        noMachine.machine = payable(address(0));
        vm.expectRevert(PluginDeployer.PluginDeployer_NoMachine.selector);
        deployer.houseConfig(noMachine);

        PluginMachine memory noStart = _machine(PluginKeep.NONE, PluginDoubling.DAILY);
        noStart.startsAtOrAfter = 0;
        vm.expectRevert(PluginDeployer.PluginDeployer_NoStartTime.selector);
