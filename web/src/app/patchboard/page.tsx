...
export default function PatchboardPage() {
  ...
  const [status, setStatus] = useState<BoardStatus>({ held: false, depth: 0.3, connected: 6, docking: false });
  ...
  useEffect(() => {
    ...
    try { controller.current = startPatchboard(canvas.current!, setStatus, saved); }
  ...
  return (
    <main className="fixed inset-0 isolate overflow-hidden bg-[#ece9e2] text-[#45483e]" style={{ fontFamily: "ui-monospace, SFMono-Regular, monospace" }}>
      ...
      <header className="pointer-events-none absolute left-6 right-6 top-5 flex items-start justify-between gap-4 sm:left-9 sm:right-9 sm:top-7">
      ...
      <details open className="absolute right-6 top-24 w-64 rounded-lg border border-black/15 bg-[#f2f0e9]/95 p-4 text-[11px] shadow-sm backdrop-blur-sm sm:right-9">
      ...
      <footer className="pointer-events-none absolute bottom-5 left-6 right-6 flex flex-wrap items-end justify-between gap-4 text-[11px] sm:bottom-7 sm:left-9 sm:right-9">
        ...
        <div className="pointer-events-auto rounded bg-[#ece9e2]/90 p-3 backdrop-blur-sm">
          <p aria-live="polite" className="opacity-60">{status.docking ? "Seating plug…" : status.held ? `Holding · depth ${status.depth.toFixed(2)}` : `${status.connected} / 6 plugs connected · Earth gravity`}</p>
        </div>
      </footer>
    </main>
  );
