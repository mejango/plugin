...
const out = await pg.evaluate(async () => {
  ...
  const way = [far, X, P]; const L = dF + rest;
  ...
  for (let k = 0; k < 150; k++) { ev('pointermove', Q.x + (k % 2), Q.y); await wait(16); maxTug = Math.max(maxTug, O.tugA || 0, O.tugB || 0); if (k % 3 === 0) handTrail.push(Math.round(Math.hypot(G.pts[0].x - Q.x, G.pts[0].y - Q.y))); if (firedAt < 0 && (O.looseA || O.looseB)) firedAt = k; s.stick.has("0,1") || s.stick.set("0,1", -1); }
  await wait(2500);
